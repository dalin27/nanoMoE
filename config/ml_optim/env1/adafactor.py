...
# $ torchrun --standalone --nproc_per_node=2 train.py config/train_moe_adafactor.py

wandb_log = True
init_from = 'scratch'
wandb_project = 'ml-optim'
wandb_run_name ='gpt2-moe-adafactor_' + time.strftime('%Y-%m-%d %H:%M:%S')
...
weight_decay = 0.0
beta1 = 0.0 # Adafactor doesn't use standard momentum
beta2 = 0.0 # Second moment estimator is factored internally
momentum = 0.0
...
decay_lr = True 
warmup_iters = 500 
learning_rate = 6e-4 # Overriding Adafactor's relative_step to match absolute baseline
min_lr = 6e-5